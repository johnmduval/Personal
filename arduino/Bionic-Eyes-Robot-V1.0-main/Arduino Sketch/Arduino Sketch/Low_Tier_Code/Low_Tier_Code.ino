...
void loop2() {
  ...
  unsigned long x = (mtime / 100) % 100;
  unsigned long xafter = (x < 51) ? x : 100 - x;
  lexpulse = map(xafter, 0, 50, 220, 440);
  if (lexpulse > 440)
    lexpulse = 440;
  else if (lexpulse < 0)
    lexpulse = 0;
  sprintf(strBuf, "mtime=%ld: before=%ld, after=%ld, lexpulse_new=%d", mtime, x, xafter, lexpulse);
  Serial.println(x);
  ...
  //sprintf(strBuf, "%d: lex=%d, rex=%d, ley=%d, rey=%d, trim = %d", mtime, lexpulse, rexpulse, leypulse, reypulse, trimval);
